...
#         im0 = imageio.imread(local_path+"/XTES_pattern_zoom_out.png")[:, :, 3]
    ...
    def get_transform(self, img):
        ...
        r1 = np.linspace(0, np.log(self.Nt / 2) / np.log(self.logbase), Nr)
        r1p = np.linspace(0, np.log(self.Nt / 2) / np.log(self.logbase), Nr)

        # this works much better if a wide range is used...
        # theta1 = np.linspace(-np.pi/2, np.pi / 2, 361)
        theta1 = np.linspace(0,np.pi,721)
        # theta1 = np.linspace(-np.pi/36,np.pi/36,11)
        r1, theta1 = np.meshgrid(r1, theta1)
